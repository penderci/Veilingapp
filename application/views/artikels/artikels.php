            ...
            <div style="height: 450px; overflow: auto;">
                <table class="table table-striped table-hover" style="font-size: 12px;">
                    <tr ng-repeat="artikel in artikels | filter: q as results">
                        <td>{{artikel.naam}}</td>
                        <td>
                            <a href="artikels/edit/{{artikel.id}}" class="btn-sm glyphicon glyphicon-pencil"
                               tooltip="Wijzig artikel" tooltip-trigger tooltip-placement="top"></a>
                        </td>
                        <td>
                            <?php if ($this->session->userdata('rol') && $this->session->userdata('rol') == '2') { ?>
                                <a href="#" ng-click="launch_dialog(artikel.id, artikel.naam)"
                                   class="btn-sm glyphicon glyphicon-trash" tooltip="Verwijder artikel" tooltip-trigger
